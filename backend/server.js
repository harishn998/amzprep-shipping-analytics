...
async function generatePDF(data, outputPath) {
  ...
  return new Promise(async (resolve, reject) => {
    ...
    try {
      ...
      const avgMo = Math.abs(sv.amount || 0) / (data.analysisMonths || 1);
      const cfRate = (sv.amount || 0) / cuft;

      // Complete data for PDF (not limited)
      const monthly = m.monthlyBreakdown || [];
      const shipMethods = m.shipMethodBreakdown || [];
      const fromZip = (m.fromZipBreakdown || []).slice(0, 5);
      const states = (data.topStates || []).slice(0, 7);

      // ========== ENHANCED HTML WITH COMPLETE MONTHLY & SHIPPING METHOD DATA ==========
      const html = `<!DOCTYPE html><html><head><meta charset="UTF-8"><style>*{margin:0;padding:0;box-sizing:border-box}@page{margin:0}body{font-family:-apple-system,sans-serif;background:linear-gradient(180deg,#000 0%,#050814 20%,#070b1a 30%,#091332 50%,#091332 100%);color:#fff;padding:30px;orphans:3;widows:3;-webkit-print-color-adjust:exact;print-color-adjust:exact}.page{page-break-after:auto;min-height:auto}.section{padding:15px 0;page-break-inside:avoid}table{page-break-before:auto}.page:last-child{page-break-after:auto}.logo{height:24px;margin-bottom:24px}.brand{font-size:28px;font-weight:700;color:#0386FE;margin-bottom:8px;text-transform:uppercase}.sub{font-size:13px;color:#94a3b8;margin-bottom:6px}.date{font-size:10px;color:#64748b;margin-bottom:28px}h2{font-size:16px;font-weight:700;margin:12px 0 10px;page-break-after:avoid;page-break-inside:avoid}.grid4{display:grid;grid-template-columns:repeat(4,1fr);gap:10px;margin-bottom:20px;page-break-inside:avoid}.grid3{display:grid;grid-template-columns:repeat(3,1fr);gap:10px;margin-bottom:20px;page-break-inside:avoid}.card{background:rgba(9,19,50,.6);border:2px solid #0386FE;border-radius:8px;padding:12px;text-align:center;min-height:85px}.card.red{border-color:#ef4444}.card.green{border-color:#10b981}.card.blue{border-color:#0386FE}.card.gray{border-color:#475569}.lbl{font-size:9px;font-weight:700;color:#0386FE;margin-bottom:8px;text-transform:uppercase}.lbl.red{color:#ef4444}.lbl.green{color:#10b981}.val{font-size:28px;font-weight:700;margin-bottom:6px}.val.sm{font-size:22px}.val.md{font-size:26px}.sub2{font-size:9px;color:#94a3b8}.cost{background:rgba(9,19,50,.6);border:2px solid #475569;border-radius:8px;padding:14px;min-height:125px}.cost.blue{border-color:#0386FE}.cost.save{background:linear-gradient(180deg,${isInc?'#dc2626':'#059669'} 0%,${isInc?'#991b1b':'#047857'} 100%);border-color:${isInc?'#ef4444':'#10b981'};text-align:center;display:flex;flex-direction:column;justify-content:center}.ct{font-size:12px;font-weight:700;margin-bottom:10px}.ct.blue{color:#0386FE}.cr{display:flex;justify-content:space-between;font-size:10px;margin-bottom:6px}.crl{color:#94a3b8}.crv{font-weight:600}.cd{border-top:1px solid #475569;margin:10px 0}.ct2{display:flex;justify-content:space-between;font-weight:700;margin-top:8px}.ctl{font-size:11px}.ctv{font-size:16px}.sva{font-size:36px;font-weight:700;margin:8px 0}.svp{font-size:20px;margin-bottom:6px;font-weight:600}.svl{font-size:10px;opacity:.9}.svs{font-size:9px;opacity:.75;margin-top:6px}table{width:100%;border-collapse:collapse;margin-bottom:12px;page-break-inside:avoid}thead{background:#0386FE;background:linear-gradient(135deg,#0386FE 0%,#0066cc 100%);page-break-after:avoid;page-break-inside:avoid}th{padding:8px;font-size:9px;font-weight:700;text-transform:uppercase;color:#fff}td{padding:6px 8px;font-size:10px;border-bottom:1px solid rgba(71,85,105,.3)}tr:nth-child(even){background:rgba(15,20,25,.4)}tr.total{background:rgba(3,134,254,.15);font-weight:700;border-top:2px solid #0386FE}.bar{margin-bottom:12px}.bi{display:flex;align-items:center;margin-bottom:8px}.bl{width:110px;font-size:11px;font-weight:600}.bt{flex:1;height:24px;background:rgba(15,20,25,.6);border-radius:4px;overflow:hidden}.bf{height:100%;background:#0386FE;border-radius:4px}.bv{width:100px;text-align:right;font-size:11px;font-weight:600;margin-left:10px}.bk{background:rgba(9,19,50,.6);border:1.5px solid #0386FE;border-radius:8px;padding:12px;margin-bottom:10px;page-break-inside:avoid}.bkt{font-size:12px;font-weight:700;margin-bottom:6px}.bkv{font-size:20px;font-weight:700;color:#0386FE;float:right}.bkd{font-size:9px;color:#94a3b8;margin-top:6px;clear:both}.rec{background:rgba(9,19,50,.6);border:2px solid;border-radius:8px;padding:12px;margin-bottom:10px;position:relative;page-break-inside:avoid}.rec.hi{border-color:#ef4444}.rec.md{border-color:#f59e0b}.badge{position:absolute;top:12px;right:12px;padding:4px 10px;border-radius:4px;font-size:8px;font-weight:700;text-transform:uppercase}.badge.hi{background:rgba(127,29,29,.8);color:#ef4444}.badge.md{background:rgba(120,53,15,.8);color:#f59e0b}.rt{font-size:13px;font-weight:700;margin-bottom:8px;padding-right:100px}.rd{font-size:10px;color:#94a3b8;line-height:1.5}.mg{display:grid;grid-template-columns:repeat(5,1fr);gap:8px;padding:12px;background:#1a1f2e;border-radius:6px}.mc{background:linear-gradient(135deg,#0386FE,#0066cc);border-radius:6px;padding:10px;text-align:center}</style></head><body>
<div style="page-break-after:auto;padding-bottom:20px">${logoBase64?`<img src="${logoBase64}" class="logo" alt="AMZ Prep">`:'<div style="font-size:18px;font-weight:700;margin-bottom:24px">amz prep</div>'}<div class="brand">${brandName.toUpperCase()}</div><div class="sub">FBA Shipping Analysis Report</div><div class="date">${new Date().toLocaleDateString('en-US',{month:'long',day:'numeric',year:'numeric'})}</div><h2>Key Metrics</h2><div class="grid4"><div class="card"><div class="lbl">Total Shipments</div><div class="val">${(data.totalShipments||0).toLocaleString()}</div><div class="sub2">${(m.totalUnits||0).toLocaleString()} units</div></div><div class="card"><div class="lbl">Total Pallets</div><div class="val">${(m.totalPallets||0).toLocaleString()}</div><div class="sub2">${Math.round(cuft).toLocaleString()} cuft</div></div><div class="card ${isInc?'red':'green'}"><div class="lbl ${isInc?'red':'green'}">${isInc?'Additional Cost':'Savings'}</div><div class="val">$${svAmt.toLocaleString()}</div><div class="sub2">${svPct.toFixed(1)}% ${isInc?'increase':'savings'}</div></div><div class="card"><div class="lbl">Transit Time</div><div class="val sm">${tCur}d → ${tAmz}d</div><div class="sub2" style="color:#10b981">-${tImp} days</div></div></div><h2>Cost Comparison</h2><div class="grid3"><div class="cost gray"><div class="ct">Current Provider</div><div class="cr"><span class="crl">Freight:</span><span class="crv">$${ccFreight.toLocaleString()}</span></div><div class="cr"><span class="crl">Placement:</span><span class="crv">$${ccPlace.toLocaleString()}</span></div><div class="cd"></div><div class="ct2"><span class="ctl">Total:</span><span class="ctv">$${ccTotal.toLocaleString()}</span></div><div class="cr" style="margin-top:8px"><span class="crl">Cost/Cuft:</span><span class="crv" style="color:#fbbf24;font-weight:700">$${ccPerCuft.toFixed(2)}</span></div></div><div class="cost blue"><div class="ct blue">AMZ Prep</div><div class="cr"><span class="crl">Middle Mile:</span><span class="crv">${pcPat}</span></div><div class="cr"><span class="crl">Internal Transfer:</span><span class="crv">$${pcInt.toLocaleString()}</span></div><div class="cd" style="border-color:rgba(3,134,254,.3)"></div><div class="ct2"><span class="ctl" style="color:#0386FE">Total:</span><span class="ctv" style="color:#0386FE">$${pcTotal.toLocaleString()}</span></div><div class="cr" style="margin-top:8px"><span class="crl">Cost/Cuft:</span><span class="crv" style="color:#0386FE;font-weight:700">$${pcPerCuft.toFixed(2)}</span></div></div><div class="cost save"><div class="ct">${isInc?'Additional Cost':'Your Savings'}</div><div class="sva">$${svAmt.toLocaleString()}</div><div class="svp">${svPct.toFixed(1)}%</div><div class="svl">vs current provider</div><div class="svs">Cuft Rate: ${isInc?'+':'-'}$${Math.abs(cfRate).toFixed(2)}</div></div></div>
</div>
${monthly.length>0?`<div style="page-break-inside:avoid;padding:20px 0"><h2>Monthly Shipment Breakdown</h2><table><thead><tr><th>MONTH</th><th style="text-align:center">SHIPMENTS</th><th style="text-align:center">DISTRIBUTION</th><th style="text-align:right">UNITS</th><th style="text-align:right">PALLETS</th><th style="text-align:right">CUFT</th><th style="text-align:right">CURRENT COST</th><th style="text-align:right;color:#0386FE">AMZ PREP COST</th><th style="text-align:right;color:#ef4444">SAVINGS</th></tr></thead><tbody>${monthly.map(x=>{const totalShip=monthly.reduce((a,m)=>a+(m.shipmentCount||0),0);const pct=totalShip>0?((x.shipmentCount/totalShip)*100).toFixed(2):0;const savings=(x.clientTotalFees||0)-(x.totalFreightCost||0);const savColor=savings<0?'#ef4444':'#10b981';return`<tr><td style="font-weight:600">${x.month}</td><td style="text-align:center">${x.shipmentCount}</td><td style="text-align:center"><span style="padding:2px 6px;background:rgba(3,134,254,.15);border-radius:3px;font-size:9px;font-weight:600;color:#0386FE">${pct}%</span></td><td style="text-align:right">${(x.qty||0).toLocaleString()}</td><td style="text-align:right">${(x.palletCount||0).toFixed(2)}</td><td style="text-align:right">${Math.round(x.totalCuft||0).toLocaleString()}</td><td style="text-align:right;font-weight:600">$${Math.round(x.clientTotalFees||0).toLocaleString()}</td><td style="text-align:right;color:#0386FE;font-weight:600">$${Math.round(x.totalFreightCost||0).toLocaleString()}</td><td style="text-align:right;font-weight:600;color:${savColor}">${savings<0?'-':''}$${Math.abs(Math.round(savings)).toLocaleString()}</td></tr>`}).join('')}${(()=>{const t=monthly.reduce((acc,x)=>({shipmentCount:acc.shipmentCount+(x.shipmentCount||0),qty:acc.qty+(x.qty||0),palletCount:acc.palletCount+(x.palletCount||0),totalCuft:acc.totalCuft+(x.totalCuft||0),clientTotalFees:acc.clientTotalFees+(x.clientTotalFees||0),totalFreightCost:acc.totalFreightCost+(x.totalFreightCost||0)}),{shipmentCount:0,qty:0,palletCount:0,totalCuft:0,clientTotalFees:0,totalFreightCost:0});const totalSav=t.clientTotalFees-t.totalFreightCost;const savColor=totalSav<0?'#ef4444':'#10b981';return`<tr class="total"><td>TOTAL</td><td style="text-align:center">${t.shipmentCount}</td><td style="text-align:center">100%</td><td style="text-align:right">${t.qty.toLocaleString()}</td><td style="text-align:right">${t.palletCount.toFixed(2)}</td><td style="text-align:right">${Math.round(t.totalCuft).toLocaleString()}</td><td style="text-align:right">$${Math.round(t.clientTotalFees).toLocaleString()}</td><td style="text-align:right;color:#0386FE">$${Math.round(t.totalFreightCost).toLocaleString()}</td><td style="text-align:right;color:${savColor}">${totalSav<0?'-':''}$${Math.abs(Math.round(totalSav)).toLocaleString()}</td></tr>`})()}</tbody></table></div>`:''}
${shipMethods.length>0?`<div style="page-break-inside:avoid;padding:20px 0"><h2>Shipping Method Distribution</h2><table><thead><tr><th>METHOD</th><th style="text-align:center">SHIPMENTS</th><th style="text-align:center">%</th><th style="text-align:right">UNITS</th><th style="text-align:right">PALLETS</th><th style="text-align:right">CUFT</th><th style="text-align:right">AVG TRANSIT</th><th style="text-align:right">CLIENT COST</th><th style="text-align:right;color:#0386FE">AMZ PREP COST</th></tr></thead><tbody>${shipMethods.map(x=>{const totalShip=shipMethods.reduce((a,m)=>a+(m.shipmentCount||0),0);const pct=totalShip>0?((x.shipmentCount/totalShip)*100).toFixed(0):0;return`<tr><td style="font-weight:600">${x.method||'Unknown'}</td><td style="text-align:center">${x.shipmentCount||0}</td><td style="text-align:center"><span style="padding:3px 8px;background:rgba(3,134,254,.15);border-radius:4px;font-weight:600;color:#0386FE">${pct}%</span></td><td style="text-align:right">${(x.qty||0).toLocaleString()}</td><td style="text-align:right">${(x.palletCount||0).toFixed(1)}</td><td style="text-align:right">${Math.round(x.totalCuft||0).toLocaleString()}</td><td style="text-align:right">${x.avgTransitTime||0}d</td><td style="text-align:right;font-weight:600">$${Math.round(x.clientTotalFees||0).toLocaleString()}</td><td style="text-align:right;color:#0386FE;font-weight:600">$${Math.round((x.mmCost||0)+(x.internalTransfer||0)).toLocaleString()}</td></tr>`}).join('')}${(()=>{const t=shipMethods.reduce((acc,x)=>({shipmentCount:acc.shipmentCount+(x.shipmentCount||0),qty:acc.qty+(x.qty||0),palletCount:acc.palletCount+(x.palletCount||0),totalCuft:acc.totalCuft+(x.totalCuft||0),clientTotalFees:acc.clientTotalFees+(x.clientTotalFees||0),mmCost:acc.mmCost+(x.mmCost||0),totalFreightCost:acc.totalFreightCost+((x.mmCost||0)+(x.internalTransfer||0)),transitSum:acc.transitSum+((x.avgTransitTime||0)*(x.shipmentCount||0))}),{shipmentCount:0,qty:0,palletCount:0,totalCuft:0,clientTotalFees:0,mmCost:0,totalFreightCost:0,transitSum:0});const avgT=t.shipmentCount>0?Math.round(t.transitSum/t.shipmentCount):0;return`<tr class="total"><td>TOTAL</td><td style="text-align:center">${t.shipmentCount}</td><td style="text-align:center">100%</td><td style="text-align:right">${t.qty.toLocaleString()}</td><td style="text-align:right">${t.palletCount.toFixed(1)}</td><td style="text-align:right">${Math.round(t.totalCuft).toLocaleString()}</td><td style="text-align:right">${avgT}d</td><td style="text-align:right">${Math.round(t.clientTotalFees).toLocaleString()}</td><td style="text-align:right;color:#0386FE">${Math.round(t.totalFreightCost).toLocaleString()}</td></tr>`})()}</tbody></table></div>`:''}
<div style="page-break-after:auto;padding-bottom:20px">${fromZip.length>0?`<div style="page-break-inside:avoid"><h2>From Zip Distribution</h2><table style="margin-bottom:16px"><thead><tr><th>ORIGIN</th><th style="text-align:center">#</th><th style="text-align:center">%</th><th style="text-align:right">DAYS</th><th style="text-align:right">UNITS</th><th style="text-align:right">CUFT</th><th style="text-align:right">PLACEMENT</th><th style="text-align:right">CARRIER</th><th style="text-align:right">CLIENT</th><th style="text-align:right;color:#0386FE">MM</th><th style="text-align:right;color:#0386FE">INTERNAL</th><th style="text-align:right;color:#10b981">FREIGHT</th></tr></thead><tbody>${fromZip.map(z=>{const totalShip=fromZip.reduce((a,x)=>a+(x.shipmentCount||0),0);const pct=totalShip>0?((z.shipmentCount/totalShip)*100).toFixed(2):0;return`<tr><td style="font-weight:600">${z.fromZip} (${z.state})</td><td style="text-align:center">${z.shipmentCount||0}</td><td style="text-align:center"><span style="padding:2px 6px;background:rgba(3,134,254,.15);border-radius:3px;font-size:9px;font-weight:600;color:#0386FE">${pct}%</span></td><td style="text-align:right">${Math.round(z.avgTransitTime||0)}</td><td style="text-align:right">${(z.qty||0).toLocaleString()}</td><td style="text-align:right">${Math.round(z.totalCuft||0).toLocaleString()}</td><td style="text-align:right">$${Math.round(z.clientPlacementFees||0).toLocaleString()}</td><td style="text-align:right">$${Math.round(z.clientCarrierCost||0).toLocaleString()}</td><td style="text-align:right;font-weight:600">$${Math.round(z.clientTotalFees||0).toLocaleString()}</td><td style="text-align:right;color:#0386FE">$${Math.round(z.mmCost||0).toLocaleString()}</td><td style="text-align:right;color:#0386FE">$${Math.round(z.internalTransfer||0).toLocaleString()}</td><td style="text-align:right;color:#10b981;font-weight:600">$${Math.round(z.totalFreightCost||0).toLocaleString()}</td></tr>`}).join('')}${(()=>{const t=fromZip.reduce((acc,x)=>({shipmentCount:acc.shipmentCount+(x.shipmentCount||0),qty:acc.qty+(x.qty||0),totalCuft:acc.totalCuft+(x.totalCuft||0),clientPlacementFees:acc.clientPlacementFees+(x.clientPlacementFees||0),clientCarrierCost:acc.clientCarrierCost+(x.clientCarrierCost||0),clientTotalFees:acc.clientTotalFees+(x.clientTotalFees||0),mmCost:acc.mmCost+(x.mmCost||0),totalFreightCost:acc.totalFreightCost+((x.mmCost||0)+(x.internalTransfer||0)),internalTransfer:acc.internalTransfer+(x.internalTransfer||0),totalFreightCost:acc.totalFreightCost+(x.totalFreightCost||0),transitSum:acc.transitSum+((x.avgTransitDays||0)*(x.shipmentCount||0))}),{shipmentCount:0,qty:0,totalCuft:0,clientPlacementFees:0,clientCarrierCost:0,clientTotalFees:0,mmCost:0,internalTransfer:0,totalFreightCost:0,transitSum:0});const totalDays=fromZip.reduce((sum,z)=>{const days=Math.round(z.avgTransitTime||0);const count=z.shipmentCount||0;return sum+(days*count)},0);const avgDays=t.shipmentCount>0?Math.round(totalDays/t.shipmentCount):0;return`<tr class="total"><td>Grand Total</td><td style="text-align:center">${t.shipmentCount}</td><td style="text-align:center">100%</td><td style="text-align:right">${avgDays}</td><td style="text-align:right">${t.qty.toLocaleString()}</td><td style="text-align:right">${Math.round(t.totalCuft).toLocaleString()}</td><td style="text-align:right">$${Math.round(t.clientPlacementFees).toLocaleString()}</td><td style="text-align:right">$${Math.round(t.clientCarrierCost).toLocaleString()}</td><td style="text-align:right">$${Math.round(t.clientTotalFees).toLocaleString()}</td><td style="text-align:right;color:#0386FE">$${Math.round(t.mmCost).toLocaleString()}</td><td style="text-align:right;color:#0386FE">$${Math.round(t.internalTransfer).toLocaleString()}</td><td style="text-align:right;color:#10b981">$${Math.round(t.totalFreightCost).toLocaleString()}</td></tr>`})()}</tbody></table></div>`:''}<h2>Summary</h2><div class="grid3"><div class="card ${isInc?'red':'green'}"><div class="lbl ${isInc?'red':'green'}">Avg Monthly</div><div class="val md">$${Math.abs(avgMo).toFixed(0)}</div><div class="sub2">${isInc?'+':'-'}$${Math.abs(ccPerCuft-pcPerCuft).toFixed(2)}/cuft</div></div><div class="card red"><div class="lbl red">YoY</div><div class="val md">${isInc?'+':'-'}${svPct.toFixed(1)}%</div><div class="sub2">${isInc?'increase':'savings'}</div></div><div class="card blue"><div class="lbl">Cost/Cuft</div><div class="val md">$${pcPerCuft.toFixed(2)}</div><div class="sub2">$${ccPerCuft.toFixed(2)} current</div></div></div>${states.length>0?`<h2>USA Volume</h2><div style="background:rgba(15,20,25,.6);border-radius:8px;padding:12px;margin-bottom:16px"><div class="mg">${states.map(s=>`<div class="mc"><div style="font-size:16px;font-weight:700;margin-bottom:2px">${s.code}</div><div style="font-size:20px;font-weight:700;margin-bottom:2px">${s.percentage}%</div><div style="font-size:9px;opacity:.8">$${s.avgCost}</div></div>`).join('')}</div></div><h2>Top States</h2><div class="bar">${states.map(s=>{const pct=parseFloat(s.percentage);return`<div class="bi"><div class="bl">${s.code}</div><div class="bt"><div class="bf" style="width:${pct}%"></div></div><div class="bv">${s.percentage}% <span style="color:#94a3b8;font-size:9px">($${s.avgCost})</span></div></div>`;}).join('')}</div>`:''}
